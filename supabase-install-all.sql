...
-- Versão 212. Execute este arquivo completo no SQL Editor do Supabase.
...
-- ARQUIVAMENTO CONTROLADO: preserva o chamado e impede exclusão acidental.
alter table public.fleet_issues drop constraint if exists fleet_issues_status_check;
alter table public.fleet_issues add constraint fleet_issues_status_check
check (status in ('aberta','aprovada','retificacao','recusada','reenviada','resolvida','arquivada'));
...
begin
  ...
  return query update public.fleet_issues as issue
    set status = 'arquivada', data = jsonb_set(coalesce(issue.data, '{}'::jsonb), '{archive}', jsonb_build_object('archivedAt', archived_at, 'reason', trim(p_reason), 'archivedBy', auth.email()), true) || jsonb_build_object('archivedAt', archived_at, 'archiveReason', trim(p_reason), 'archivedBy', auth.email())
...
begin
  ...
  return query update public.fleet_issues as issue
    set status = 'aberta', data = (coalesce(issue.data, '{}'::jsonb) - 'archivedAt' - 'archiveReason' - 'archivedBy') || jsonb_build_object('archive', coalesce(issue.data->'archive','{}'::jsonb) || jsonb_build_object('restoredAt', restored_at, 'restoredBy', auth.email()))
    where issue.id::text = p_issue_id and issue.status = 'arquivada'
    returning issue.data;
