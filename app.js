...
function notifyReturnedIssue(issue) {
  ...
  localStorage.setItem(key, new ction baseKey(value) { const text = String(value || "").toLowerCase(); if (text.includes("vertical")) return "Vertical"; if (text.includes("horizontal")) return "Horizontal"; if (text.includes("abrigo")) return "Abrigo"; if (text.includes("sasc")) return "SASC"; if (text.includes("gestão") || text.includes("gestao")) return "Gestão"; return ""; }
...
async function saveMaintenance() {
  ...
  if (issue.maintenance.status === "Agendada") { await sendDriverMaintenanceWhatsApp(issue); sendSchedulingReturn(issue, issue.maintenance); }
