...
async function syncLocalBacklog() {
  ...
  try {
    await Promise.all((data.inspections || []).map((inspection) => cloudSave("fleet_inspections", { id: inspection.id, vehicle_id: inspection.vehicleId, data: inspection })));
    await Promise.all(data.issues.map((issue) => cloudSave("fleet_issues", { id: issue.id, inspection_id: issue.inspectionId, vehicle_id: issue.vehicleId, status: issue.status || "aberta", data: issue })));
