...
async function loadMasterAccess() {
  ...
  try {
    ...
    if (!managementRole) {
      ...
      location.replace("gestao.html?v=174&acesso=negado");
      return;
    }
  } catch (error) {
...
function leadershipPanelUrl(baseName) {
  ...
  return `${location.origin}${location.pathname.replace(/[^/]*$/, "lider.html")}?v=174&base=${encodeURIComponent(base)}`;
}
async function approvalUrl(vehicle, issues) {
  ...
  return `${location.origin}${location.pathname.replace(/[^/]*$/, "aprovacao.html")}?v=174&${params.toString()}`;
}
...
function sendLeaderInstall() {
  ...
  const link = `https://4lves-dev.github.io/checkfrota/instalar-lider.html?v=174&base=${encodeURIComponent(base)}`;
...
if ("serviceWorker" in navigator) window.addEventListener("load", async () => {
  ...
  try { const registration = await navigator.serviceWorker.register("service-worker.js?v=174"); await registration.update(); } catch (_) {}
});
...
if (new URLSearchParams(location.search).get("gestao") === "1") {
  ...
  else location.replace("gestao.html?v=174");
} else { renderStart(); }
