...
function leadershipPanelUrl(baseName) {
  ...
  return `${location.origin}${location.pathname.replace(/[^/]*$/, "lider.html")}?v=125&base=${encodeURIComponent(base)}`;
}
...
function sendLeaderInstall() {
  ...
  const link = `https://4lves-dev.github.io/checkfrota/instalar-lider.html?v=125&base=${encodeURIComponent(base)}`;
...
if ("serviceWorker" in navigator) window.addEventListener("load", () => navigator.serviceWorker.register("service-worker.js?v=126").catch(() => {}));
